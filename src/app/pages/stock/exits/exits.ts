...
@Component({
  ...
  template: `
    <div class="dashboard-container">
      <app-sidebar activePage="exits"></app-sidebar>

      <main class="content">
        <header class="content-header">
          <div>
            <h1>Saídas</h1>
            <p>Registre novas saídas de mercadoria.</p>
          </div>
        </header>
        ...
        <div class="form-container">
          <form [formGroup]="exitForm" (ngSubmit)="onSubmit()">
            ...
            <div class="form-group">
              <label>Data</label>
              <input type="date" formControlName="date_exit">
            </div>
            <div class="form-group">
              <label>Hora</label>
            ...
            <button type="submit" class="btn-primary" [disabled]="exitForm.invalid || loading()">
...
export class ExitsComponent implements OnInit {
  ...
  exitForm: FormGroup = this.fb.group({
    ...
    date_exit: [new Date().toISOString().split('T')[0], [Validators.required]],
  ...
  ngOnInit() {
    if (this.authService.user()) {
      this.exitForm.patchValue({ company_id: this.authService.user()?.company_id });
    }
  }
  ...
  selectItem(item: Item) {
    ...
    this.exitForm.patchValue({ item_id: item.id, price: item.price });
    this.itemSuggestions.set([]);
  }
  ...
  async onSubmit() {
    ...
    try {
      await this.stockService.createExit(this.exitForm.value);
      this.modalMessage.set('Saída registrada com sucesso!');
      this.showMessageModal.set(true);
      this.exitForm.reset({ company_id: this.authService.user()?.company_id, date_exit: new Date().toISOString().split('T')[0], hour: new Date().toTimeString().split(' ')[0].substring(0, 5) });
      this.selectedItemName.set('');
    } catch (e: any) {
      this.modalMessage.set(e?.error?.detail || 'Erro ao registrar saída!');
      this.showMessageModal.set(true);
    } finally {
